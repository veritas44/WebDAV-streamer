    ...
    <div class="row">
        ...
        <div class="col-xs-9">
            <div class="input-group" style="margin: 10px 0; float: right">
                ...
                <button href="javascript:;" class="btn btn-default btn-sm" onclick="loadPage('search.php?folder=' + currentDirectory)" title="Search a file in this folder"><span class="glyphicon glyphicon-search"></span></button>
                <button href="javascript:;" class="btn btn-default btn-sm" onclick="addAllToPlaylist()" title="Add all music files in this folder and subfolders to the playlits"><span class="glyphicon glyphicon-plus-sign"></span></button>
            </div>
        </div>
    </div>
...
try {
    ...
    <div>
        <table id='filebrowserTable' class="table table-striped table-hover" style="width: 100%;">
            ...
            <td><a href='javascript:;' onclick='playAudio(\"" . Sabre\HTTP\encodePath($key) . "\", \"" . urlencode(readable_name($key)) . "\")'>" . readable_name($key) . "</a></td>
            <td width='75px' class='table-icon' align=\"right\">
                <a class='btn btn-xs btn-default' href='javascript:;' onclick='addToPlaylist(\"" . Sabre\HTTP\encodePath($key) . "\", \"" . urlencode(readable_name($key)) . "\")' title='Add to playlist'><span class='glyphicon glyphicon-plus-sign'></span></a> <a class='btn btn-xs btn-default' href='javascript:;' onclick='addFavourite(\"" . urlencode($key) . "\", \"" . urlencode(readable_name($key)) . "\", \"audio\")' title='Favourite this audio'><span class='glyphicon glyphicon-star'></span></a>
            </td>
            </tr>";
                    }
